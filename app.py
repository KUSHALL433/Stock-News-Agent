...
def business_scraper(state:AgentState):
    ...
    writer({"custom_key":"Scrapping Business Article"})
    ...
    writer({"custom_key":"Business Article Scrapping done!."})
    return {"business_articles": state["business_articles"]}

def economy_scraper(state:AgentState):
    from moneycontrol import get_article_links,scrape_article
    writer=get_stream_writer()
    writer({"custom_key":"Scraping Economy Articles"})
    ...
    writer({"custom_key":"Economy Articles Scrapping done!."})
    return {"economy_articles":state['economy_articles']}

def companies_scraper(state:AgentState):
    from moneycontrol import get_article_links,scrape_article
    writer=get_stream_writer()
    writer({"custom_key":"Scrapping Company Articles"})
    ...
    writer({"custom_key":"Company Articles Scrapping done!."})
...
def ai_summary(state:AgentState):
    combined_text = ""
    writer=get_stream_writer()
    writer({"custom_key":"Generating Summary from all the sources."})
    ...
    writer({"custom_key":"Summary Generation Successful!!"})
    return state
